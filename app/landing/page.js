...
const CurvedScrollableNavigation = () => {
  ...
  const getNavItemStyles = (index, isActive) => {
    ...
    return {
      transform: isActive
        ? `translateX(0px) scale(${windowWidth < 768 ? "1.1" : "1.2"})`
        : `translateX(-${
            windowWidth < 768 ? horizontalOffset / 2 : horizontalOffset
          }vh) scale(${windowWidth < 768 ? "0.9" : "0.8"})`,
    ...
    <div className="h-screen flex flex-col md:flex-row bg-[#0c102998] text-white overflow-hidden">
      ...
      {windowWidth > 0 && (
        <>
          ...
          {/* left */}
          <div
            className={`left w-full md:w-1/2 ${isMobileMenuOpen
